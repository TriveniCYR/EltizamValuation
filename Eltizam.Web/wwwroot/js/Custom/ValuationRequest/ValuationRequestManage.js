        ...
        success: function (response) {
            if (response != null) { 
                $.each(response._object, function (index, object) {
                    ...
                    $('#InvoiceTable tbody').append(' <tr id="' + object.id + '"><td><a href=' + url + '>' + object.referenceNO + '</a></td><td>' + object.invoiceNo + '</td><td>' + moment(object.transactionDate).format('DD-MMM-YYYY') + '</td><td class="formatting">' + object.amount + '</td><td>' + object.userName + '</td><td>' + moment(object.createdDate).format('DD-MMM-YYYY') + '</td><td>' + html + '</td></tr>');
                });
                formatCurrencyInElements('formatting');
            }
        },
